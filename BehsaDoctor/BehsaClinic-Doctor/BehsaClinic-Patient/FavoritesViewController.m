...
- (void)tapOnAuthorImage:(UITapGestureRecognizer *)tap{
    //NSDictionary *tempDic = [self.tableArray myObjectAtIndex:tap.view.tag];
    UIStoryboard *story = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if ([self.tableArray count] == 0) {
        ...
        noResultLabel.font = FONT_NORMAL(13);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row < [self.tableArray count]) {
        NSDictionary *searchDic = [self.tableArray myObjectAtIndex:indexPath.row];
        cell.nameLabel.text = searchDic.LPTVTitle;//[[self.tableArray myObjectAtIndex:indexPath.row]objectForKey:@"title"];
        cell.jobLabel.text = searchDic.LPTVUserTitle;//[[[self.tableArray myObjectAtIndex:indexPath.row]objectForKey:@"entity"]objectForKey:@"name"];
        [cell.myImageView setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@", searchDic.LPTVImageUrl]] placeholderImage:[UIImage imageNamed:@"dontprofile"]];
    }
    return cell;
}
...
- (void)likeOnServerWithID:(NSString *)idOfPost{
    ...
    if ([userpassDic count] > 1) {
        ...
        params = @{@"username":[userpassDic myObjectForKey:@"username"]/*@"09191234072"*/,
                   @"password":[userpassDic myObjectForKey:@"password"]/*@“123456”*/,
                   @"model": @"POST",
                   @"foreign_key": idOfPost,
                   @"user_id": [NSNumber numberWithInteger:[GetUsernamePassword getUserId]]/*@"6144"*/,
                   @"unit_id": @"3"
                   };
    }else{
        params = @{@"username":@"",
    ...
    [manager POST:url parameters:params progress:nil success:^(NSURLSessionTask *operation, id responseObject) {
        ...
        if ([[tempDic myObjectForKey:@"status"] isEqualToString:@"+"]) {
...
- (void)favOnServerWithID:(NSString *)idOfPost{
    ...
    NSDictionary *userpassDic = [GetUsernamePassword getUsernamePassword];
    NSDictionary *params = @{@"username":[userpassDic myObjectForKey:@"username"]/*@"09191234072"*/,
                             @"password":[userpassDic myObjectForKey:@"password"]/*@“123456”*/,
                             @"model": @"POST",
                             @"foreign_key": idOfPost,
                             @"user_id": [NSNumber numberWithInteger:[GetUsernamePassword getUserId]]/*@"6144"*/,
