...
@interface SessionDetailViewController ()<UITextViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIGestureRecognizerDelegate, AVAudioRecorderDelegate, AVAudioPlayerDelegate, UIScrollViewDelegate>
{
    ...
    UIButton *playSoundButtonPreview;
...
- (void)scrollviewMaker{
    ...
    UIButton *finishThreatment = [CustomButton initButtonWithTitle:@"پایان دوره درمان" withTitleColor:[UIColor whiteColor] withBackColor:MAIN_COLOR isRounded:NO withFrame:CGRectMake(-5, screenHeight - 38, screenWidth + 10,40)];
...
- (void)parvandeButtonAction{
    [self dismissViewControllerAnimated:YES completion:nil];
    [[NSUserDefaults standardUserDefaults]setObject:@"YES" forKey:@"dismissAllSession"];
}
...
- (void)makeNeveshtaneSharhehalView{
    ...
    [borderView addSubview:_textView];
        ...
        //30 × 54
    recorderButton = [[UIButton alloc]initWithFrame:CGRectMake(20, borderView.frame.size.height - 35, 64 * 0.5, 64 * 0.5)];
    ...
    [borderView addSubview:recorderButton];
        ...
        //43 × 47
    attachButton = [[UIButton alloc]initWithFrame:CGRectMake(60, borderView.frame.size.height - 28, 43 * 0.5, 47 * 0.5)];
...
- (void)makeHealthRecordsView:(NSArray *)items{
    ...
    for (NSInteger i = 0; i < [items count]; i++) {
        ...
        NSString *dateStr = [[items myObjectAtIndex:i] myObjectForKey:@"created_at"];
        ...
        if ([attachment length] > 10) {
            CGFloat ratio = [[[items myObjectAtIndex:i] myObjectForKey:@"attachment_ratio"]floatValue];
            UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(50, ypos + 10, screenWidth - 100, (screenWidth - 100) * ratio)];
            ...
            [imageView setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@.png", attachment]] placeholderImage:[UIImage imageNamed:@"format_png"]];
        ...
        if ([voice length] > 10) {
            ...
            UIView *greenBGView = [[UIView alloc]initWithFrame:CGRectMake(5, ypos + 10, screenWidth - 10, 40)];
            greenBGView.backgroundColor = PREVIEW_COLOR;
            greenBGView.layer.cornerRadius = 20;
            greenBGView.clipsToBounds = YES;
            [bgViewForHealthRecords addSubview:greenBGView];
            ...
            ypos = greenBGView.frame.origin.y + greenBGView.frame.size.height + 5;
        }
...
- (void)tapImageView:(UITapGestureRecognizer *)tap{
    ...
    NSString *url = [NSString stringWithFormat:@"%@.png", [tempDic  myObjectForKey:@"attachment"]];
...
- (void)playAudioForHealthRecord:(id)sender{
    ...
    if ([healthRecordsItems count] > 0) {
        NSDictionary *tempDic = [healthRecordsItems objectAtIndex:btn.tag];
        UIStoryboard *story = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
        ...
        NSString *url = [NSString stringWithFormat:@"%@.mp3", [tempDic  valueForKey:@"voice"]];
        view.urlString = url;
        [self presentViewController:view animated:YES completion:nil];
    }
...
- (void)disableAttachButton{
    ...
    [attachButton setBackgroundImage:[UIImage imageNamed:@"attach file-disable.png"] forState:UIControlStateNormal];
    attachButton.userInteractionEnabled = NO;
}

- (void)enableAttachButton{
    //[attachButton setHidden:NO];
    [attachButton setBackgroundImage:[UIImage imageNamed:@"attach file.png"] forState:UIControlStateNormal];
    attachButton.userInteractionEnabled = YES;
}

- (void)disableVoiceButton{
    //[recorderButton setHidden:YES];
    [recorderButton setBackgroundImage:[UIImage imageNamed:@"voice-icon-disable.png"] forState:UIControlStateNormal];
    recorderButton.userInteractionEnabled = NO;
}

- (void)enableVoiceButton{
    //[recorderButton setHidden:NO];
    [recorderButton setBackgroundImage:[UIImage imageNamed:@"voice icon.png"] forState:UIControlStateNormal];
...
- (void)recorderButtonStopAction{
    ...
    playSoundButtonPreview = [[UIButton alloc]initWithFrame:CGRectMake(5, 3, 170 * 0.2, 170 * 0.2)];
    [playSoundButtonPreview setBackgroundImage:[UIImage imageNamed:@"play.png"] forState:UIControlStateNormal];
    [playSoundButtonPreview addTarget:self action:@selector(playAudio) forControlEvents:UIControlEventTouchUpInside];
    [greenBGView addSubview:playSoundButtonPreview];
    
    CGFloat ratio = 1155 / greenBGView.frame.size.width;
    ratio /= 20;
    UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(playSoundButtonPreview.frame.size.width + 15, 7, 1155 * ratio, 102 * ratio)];
    imageView.image = [UIImage imageNamed:@"wave"];
    [greenBGView addSubview:imageView];
...
- (void)playAudio{
    ...
    [playSoundButtonPreview setBackgroundImage:[UIImage imageNamed:@"pause.png"] forState:UIControlStateNormal];
    [playSoundButtonPreview addTarget:self action:@selector(stopAudio) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)deleteAudio{
    ...
    NSString *str = @"نوشتن شرح حال...";
    if ([_textView.text isEqualToString:str]) {
        [self disableSaveButton];
    }
...
- (void)sendToServer{
    ...
    NSString *url = [NSString stringWithFormat:@"%@online_booking/submit_health_record", BaseURL];
    ...
    [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
                [saveChangesButton setTitle:[NSString stringWithFormat:@"%f", uploadProgress.fractionCompleted] forState:UIControlStateNormal];
...
- (void)getSessionDetail{
    ...
    [Networking formDataWithPath:@"online_booking/doctor/visit" parameters:params success:^(NSDictionary * _Nonnull responseDict) {
        ...
        if ([[responseDict  myObjectForKey:@"success"]integerValue] == 1) {
            ...
            //[[responseDict  myObjectForKey:@"data"] myObjectForKey:@"health_records"];
