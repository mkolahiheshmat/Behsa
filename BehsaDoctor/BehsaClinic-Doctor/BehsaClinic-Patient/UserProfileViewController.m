...
- (void)makeInfoView{
    ...
    descriptionLabel.adjustsFontSizeToFitWidth = YES;
        //descriptionLabel.text = @"تست متن تست متن تست متن تست متن تست متن تست متنتست متن تست متن تست متنتست متن تست متنتست متن تست متن تست متنتست متنتست متنتست متن تست متن تست متنتست متن تست متن تست متنتست متنتست متن  تست متن  تست متنتست متن تست متن تست متن تست متن تست متن تست متن تست متن تست متن تست متن تست متنتست متن";
...
- (void)tapOnAuthorImage:(UITapGestureRecognizer *)tap{
    ...
    view.entityID = [tempDic.LPUserEntityId integerValue];//[[[tempDic myObjectForKey:@"entity"]objectForKey:@"id"]integerValue];
...
- (void)commentImageViewAction:(UITapGestureRecognizer *)tap{
    ...
    view.postId = [[[_tableArray myObjectAtIndex:tap.view.tag]objectForKey:@"id"]integerValue];
...
{
    [ProgressHUD show:@""];
    NSString *url = [NSString stringWithFormat:@"%@profile/doctor", BaseURL];
    NSDictionary *params = @{@"id":[NSNumber numberWithInteger:_entityID]};
    ...
    [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        aboutTextView.text = [NSString stringWithFormat:@"%@", [[dictionary myObjectForKey:@"data"]objectForKey:@"about"]];
        jobTitleLabel.text = [NSString stringWithFormat:@"%@", [[[dictionary myObjectForKey:@"data"]objectForKey:@"job_title"]objectForKey:@"name"]];
        [profileImageView setImageWithURL:[NSURL URLWithString:
                                           [NSString stringWithFormat:@"%@", [[dictionary myObjectForKey:@"data"]objectForKey:@"avatar"]]]
                         placeholderImage:[UIImage imageNamed:@"icon upload ax"]];
        usernameLabel.text = [[dictionary myObjectForKey:@"data"]objectForKey:@"name"];
        [self makeElementsOFInfoView:[[dictionary myObjectForKey:@"data"]objectForKey:@"options"]];
        
        if ([[[dictionary myObjectForKey:@"data"]objectForKey:@"user_id"]integerValue] == [GetUsernamePassword getUserId]) {
...
- (void)fetchPostsFromServer:(NSInteger)pageOf{
    if (!isBusyNow) {
        ...
        NSDictionary *params = @{
                                 @"id":[NSNumber numberWithInteger:_entityID],
    ...
    [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if ([[[tempDic myObjectForKey:@"data"]objectForKey:@"status"] isEqualToString:@"+"]) {
            ...
            if (idOfTargetDelete < [_likeArray count]) {
                ...
                [Database updateLandingPageWithFilePath:[Database getDbFilePath] SetValue:[NSString stringWithFormat:@"%@", [[tempDic myObjectForKey:@"data"]objectForKey:@"count"]] field:@"likes_count" postId:idOfPost];
