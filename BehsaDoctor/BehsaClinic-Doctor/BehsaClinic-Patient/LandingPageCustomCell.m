...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier isPostImage:(BOOL)isPostImage height:(CGFloat)height coverRatio:(CGFloat)coverRatio{
    ...
    if (self) {
        [self initView:(BOOL)isPostImage height:(CGFloat)height coverRatio:(CGFloat)coverRatio];
    }
    return self;
}
...
- (LandingPageCustomCell *)setEntity:(NSDictionary *)entity indexPath:(NSIndexPath *)indexPath{
    ...
    _dateLabel.text = [NSString stringWithFormat:@"%@",
        ...
        //post image
    if ([entity.LPTVImageUrl length] > 10) {
        ...
        [_postImageView setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@", entity.LPTVImageUrl]]];
    }
    ...
    _authorJobLabel.text = entity.LPTVUserJobTitle;
        ...
        //content
    _contentLabel.text = entity.LPTVContentSummary;
...
- (void)initView:(BOOL)isPostImage height:(CGFloat)height coverRatio:(CGFloat)coverRatio{
    ...
    self.titleLabel.font = FONT_NORMAL(16);
    ...
    self.contentLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, self.titleLabel.frame.origin.y + self.titleLabel.frame.size.height, screenWidth - 30, height)];
    ...
    if (!isPostImage) {
        ...
        //52 × 49
    self.heartButton = [[UIButton alloc]initWithFrame:CGRectMake(20, _postImageView.frame.origin.y+ _postImageView.frame.size.height + 5+ 5, 52 * 0.4, 49*0.4)];
    [self.heartButton setImage:[UIImage imageNamed:@"like icon"] forState:UIControlStateNormal];
    [self addSubview:self.heartButton];
    
    self.likeCountLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.heartButton.frame.origin.x + self.heartButton.frame.size.width + 2, _postImageView.frame.origin.y+ _postImageView.frame.size.height + 5+ 5, 20, 20)];
    ...
    [self addSubview:self.likeCountLabel];
        ...
        //46 × 49
    self.commentImageView = [[UIImageView alloc]initWithFrame:CGRectMake(self.likeCountLabel.frame.origin.x + self.likeCountLabel.frame.size.width + 10, _postImageView.frame.origin.y+ _postImageView.frame.size.height + 5+ 5, 46*0.4, 49*0.4)];
    self.commentImageView.image = [UIImage imageNamed:@"comment"];
    [self addSubview:self.commentImageView];
    
    self.commentCountLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.commentImageView.frame.origin.x + self.commentImageView.frame.size.width + 2, _postImageView.frame.origin.y+ _postImageView.frame.size.height + 5+ 5, 20, 20)];
    ...
    [self addSubview:self.commentCountLabel];
        ...
        //46 × 43
    self.favButton = [[UIButton alloc]initWithFrame:CGRectMake(self.commentCountLabel.frame.origin.x + self.commentCountLabel.frame.size.width + 10, _postImageView.frame.origin.y+ _postImageView.frame.size.height + 5+ 5, 46 * 0.4, 43 * 0.4)];
    [self.favButton setImage:[UIImage imageNamed:@"fav off"] forState:UIControlStateNormal];
    [self addSubview:self.favButton];
    
        //35 × 44
    self.shareButton = [[UIButton alloc]initWithFrame:CGRectMake(self.favButton.frame.origin.x + self.favButton.frame.size.width + 18, _postImageView.frame.origin.y+ _postImageView.frame.size.height + 5+ 5, 35*0.4, 44*0.4)];
