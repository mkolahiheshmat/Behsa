...
//const CGFloat		kMessageViewHeight		= 24.0;
...
- (UIColor *)errorViewColor {
    return [UIColor colorWithRed:0.872 green:0.207 blue:0.182 alpha:1.0];
}
...
- (UIColor *)infoViewColor {
    return [UIColor colorWithRed:0.174 green:0.774 blue:0.368 alpha:1.0];
}
...
- (UIColor *)warningViewColor {
    return [UIColor colorWithRed:0.933 green:0.542 blue:0.069 alpha:1.0];
}
...
- (void)showMessage:(NSString *)message withTextColor:(UIColor *)tColor withViewColor:(UIColor *)vColor {
    ...
    messageView = [[UIView alloc] initWithFrame:CGRectMake(0, 44 + [UIApplication sharedApplication].statusBarFrame.size.height, window.frame.size.width, 0)];
    ...
    //	UIImageView *imageview = [[UIImageView alloc] initWithFrame:CGRectMake(fixedWidth - imageHeight - 8, height / 2 - imageHeight/2, imageHeight, imageHeight)];
    //	imageview.image = [UIImage imageNamed:@"Collapse"];
    ...
    [timer invalidate];
    timer = nil;
    timer = [NSTimer scheduledTimerWithTimeInterval:kDefaultTimeInterval * 0.7 target:self selector:@selector(dismissMessageView) userInfo:nil repeats:NO];
}
